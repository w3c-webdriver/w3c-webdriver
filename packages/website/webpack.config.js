...
const commonConfig = {
  resolve: {
    alias: {
      typeDoc$: path.resolve(__dirname, '../w3c-webdriver/src/index.ts'),
      packageJson$: path.resolve(__dirname, '../w3c-webdriver/package.json')
    }
  },
  output: {
    path: distPath
  ...
  module: {
    rules: [
      ...
      {
        test: /\.ts$/,
        include: path.resolve(__dirname, '../w3c-webdriver/src'),
        use: [
          {
            loader: path.resolve(__dirname, 'src/loaders/typedoc-loader.js'),
            options: {
              // mode: 'modules',
              tsConfig: path.resolve(__dirname, '../w3c-webdriver/tsconfig.json')
            }
          }
        ]
      },
